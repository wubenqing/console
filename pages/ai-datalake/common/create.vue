      ...
      <div class="p-6 bg-white rounded-lg shadow-sm border">
        ...
        <div class="mb-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- 数据集英文名称 -->
            <div>
              <label class="block text-sm font-medium mb-1"> * 数据集名称 </label>
              <Input v-model="formData.name" required placeholder="请输入名称" />
            </div>

            <!-- 数据集格式 -->
            <div>
              <label class="block text-sm font-medium mb-1"> 数据集格式 </label>
              <div class="flex flex-wrap gap-2">
                ...
                >
                  <Icon :icon="getFormatIcon(format)" class="w-4 h-4 mr-1" />
                  {{ format }}
                </Button>
              </div>
            </div>
        ...
        <div class="mb-8">
          <h3 class="text-lg font-medium mb-4 flex items-center cursor-pointer" @click="toggleAdvanced">
            <Icon :icon="advancedOpen ? 'mdi:chevron-up' : 'mdi:chevron-down'" class="w-5 h-5 mr-2" />
            高级设置
          </h3>
          ...
          <div v-show="advancedOpen" class="space-y-4">
            ...
            <div class="flex items-center gap-2">
              <label class="block text-sm font-medium mb-1"> * 所属项目 </label>
              <div class="flex-1">
      ...
      <div class="p-4">
        ...
        <div v-if="!selectedCatalog">
          ...
          <div class="border rounded-md max-h-80 overflow-y-auto">
            ...
            <div v-else class="p-8 text-center text-gray-500">暂无可用 Catalog</div>
          </div>
        </div>
        ...
        <div v-else-if="!selectedSchema">
          ...
          <div class="border rounded-md max-h-80 overflow-y-auto">
            ...
            <div v-else class="p-8 text-center text-gray-500">暂无可用 Schema</div>
          </div>
        </div>
        ...
        <div v-else>
          ...
          <div class="border rounded-md max-h-80 overflow-y-auto">
            ...
            <div v-else class="p-8 text-center text-gray-500">暂无可用 Table</div>
          </div>
        </div>
...
const filteredCatalogs = computed(() => {
  ...
  return catalogs.value.filter(catalog => catalog.name.toLowerCase().includes(catalogSearchTerm.value.toLowerCase()))
})
...
const filteredSchemas = computed(() => {
  ...
  return schemas.value.filter(schema => schema.name.toLowerCase().includes(schemaSearchTerm.value.toLowerCase()))
})
...
const filteredTables = computed(() => {
  ...
  return tables.value.filter(table => table.name.toLowerCase().includes(tableSearchTerm.value.toLowerCase()))
})
